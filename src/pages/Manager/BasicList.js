...
class BasicList extends PureComponent {
  ...
  render() {
    ...
    const ListContent = ({ data: { owner, videoTime, percent } }) => (
      <div className={styles.listContent}>
        ...
        <div className={styles.listContentItem}>
          <span>视频时长</span>
          <p>{moment(videoTime).format('YYYY-MM-DD HH:mm')}</p>
        </div>
        <div className={styles.listContentItem}>
      ...
      return (
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem label="视频地址" {...this.formLayout}>
            {getFieldDecorator('videoUrl', {
              rules: [{ required: true, message: '请输入视频地址' }],
              initialValue: current.videoTime ? moment(current.videoTime) : null,
            })(<Input placeholder="请输入" />)}
          </FormItem>
          <FormItem {...this.formLayout} label="视频描述">
            {getFieldDecorator('subDescription', {
              rules: [{ message: '请输入至少五个字符的视频描述！', min: 5 }],
